...
class ExperimentSetting():
    ...
    def run_experiment(self,branch_probability_threshold,df,x_columns,y_column,feature_types,hyper_parameters_dict):
        for i in range(self.num_of_iterations):
            ...
            output_path = 'pickles_100trees/' + str(hyper_parameters_dict['df_name']) + '_' + str(result_dict['iteration'])
            ...
            # Performance of pruned Random Forest with DISC pruning
            #start_temp = datetime.datetime.now()
            #selected_ensemble= DISC_pruning(rf,trainAndValidation_x,trainAndValidation_y,minimal_forest_size)
            #result_dict['pruning time']=  (datetime.datetime.now() - start_temp).total_seconds()
            #for index in sorted(selected_ensemble, reverse=True):
            #   del rf.estimators_[index]
           # result_dict.update(self.ensemble_measures(test_x,test_y,rf))
